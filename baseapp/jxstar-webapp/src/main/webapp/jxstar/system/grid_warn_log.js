Jxstar.currentPage = function() {
	...
	var cols = [
	{col:{header:'开始时间', width:129, sortable:true, renderer:function(value) {
			return value ? value.format('Y-m-d H:i:s') : '';
		}}, field:{name:'warn_log__start_date',type:'date'}},
	{col:{header:'结束时间', width:126, sortable:true, renderer:function(value) {
			return value ? value.format('Y-m-d H:i:s') : '';
	...
	{col:{header:'上报ID', width:100, sortable:true, hidden:true}, field:{name:'warn_log__warn_id',type:'string'}},
	{col:{header:'日志ID', width:100, sortable:true, hidden:true}, field:{name:'warn_log__log_id',type:'string'}}
	];
