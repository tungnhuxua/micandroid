...
<div class="zhuti">
				<div class="gerenshoucangbt"><span class="hyguanglibt">Firend_好友_(共 <span style="color: #91BF29; font-size: 18px;">${page.totalCount}</span> 位好友)</span><span class="yhyjifeng"><a href="member-request.action">_邀请好友注册积分</a></span></div>
				<s:if test="countRequestFriend != 0">
					<div><a href="member-friend-requestlist.action">您有  <span style="color: #91BF29; font-size: 18px;">${countRequestFriend }  </span>位朋友正在请求您加为好友</a></div>
				</s:if>
				...
				<div class="grhaohougl">
				 ...
				 %>
					<dl>
						<dt>
							<a target="_blank" href="${ctx}/blog/blog-home.action?tomember.id=<%=object[8]%>">
						...
						<dd><a href="javascript:_operate(friend_delete,'<%=object[9]%>','<%=object[8]%>');">删除好友</a>|<a  id="upblog_<%=object[9]%>" onclick="a_blogIndex('<%=object[9]%>');" style="cursor: pointer;<%if(object[10].toString().equals("0")){out.print("display:none;");}%> ">取消放置个人首页</a><a  id="unupblog_<%=object[9]%>" onclick="a_unblogIndex('<%=object[9]%>');" style="cursor: pointer;<%if(object[10].toString().equals("1")){out.print("display:none;");}%>">放置个人首页</a></dd>
					</dl>
				<%} %>
				</div>
					...
					<input name="member.id" value="${member.id}" type="hidden">
				</form>
				<div class="qyyear">
