...
<script type="text/javascript">
	var circle_member_delete = {action:"member-circle!delCircleMember.action",msg:"确定要删除该会员吗？"};
	function delCircleMember(op,id,mcuid) {
		...
		<div class="main">
		  <form  name="circleMemberFrom">
			<div class="mainbh">
				...
				<div class="zhuti">
				<div class="gerenshoucangbt"><span class="hyguanglibt">Friend_圈子   圈子共<span style="color: #91BF29; font-size: 18px;"> ${page.totalCount} </span>位会员)</span></div>
				
				   <div class="zdyhaoyou">注_当前是您圈子的所有会员 ，您是管理员！</div>
					   <div class="grhaohougl">
							 <%
								...
								<dl>
									...
									<dd><a href="javascript:delCircleMember(circle_member_delete,'<%=object[7]%>','<%=object[9]%>')">请出圈子</a></dd>
								</dl>
							<%} %>
						</div>
						...
						<div class="qyzhongjianyeshu">
							<ul>
								...
								<s:if test="page.hasPre">
									<a href="${ctx }/blog/member-circle-manager.action?id=${id}&&page.pageNo=${page.prePage}">上一页</a>
								</s:if>
								<ouun:pageNum pageNo="${page.pageNo}" url="${ctx}/blog/member-circle-manager.action?id=${id}&&page.pageNo=" totalPages="${page.totalPages}"/>
								<s:if test="page.hasNext">
									<a href="${ctx }/blog/member-circle-manager.action?id=${id}&&page.pageNo=${page.nextPage}">下一页</a>
								</s:if>
							</ul>
						</div>
						<div class="qyzhongjianyeshu">_共${page.totalCount}个好友加入</div>
					  </div>
				</div>
		    </div>
