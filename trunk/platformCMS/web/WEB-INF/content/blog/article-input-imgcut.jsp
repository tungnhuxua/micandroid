...
<div style="">
	...
	<div style="float:left;">
		<div id="imgDiv" style="overflow:hidden;width:310px;height:310px;border:1px solid #333;">
			<img id="minImg" src="${ctx}<%=request.getParameter("imagePath") %>"/>
		</div>
		<form id="submitForm" action="upload!UserLogImgCut.action" target="hiddenIframe" method="post" id="cutImgForm" style="margin:7px 0px 0px 0px;padding:0px">
			...
			<input type="hidden" name="article.imageLink" value="<%=request.getParameter("imagePath")%>"/>
			<input name="fileGroup" value="cap" type="hidden" />
			<input type="hidden" name="entity.id" value="${id}" /> 
			<div style="margin-bottom:3px;padding-top: 50px">
...
<script type="text/javascript">
	...
	$(window).load(function () {
		...
		var minSelW = Math.round(310*scale);
		var minSelH = Math.round(310*scale);
		$('img#ferret').imgAreaSelect({selectionOpacity:0,outerOpacity:'0.5',selectionColor:'blue',onSelectChange:preview,minWidth:minSelW,minHeight:minSelH,aspectRatio:'310:310',x1:0,y1:0,x2:310,y2:310});
		showCut(310,310,0,0);
	});
