...
<title>专题-${article.category.name }-${article.title }</title>
...
<div id="postion">
<p><span>您的位置:首页&gt;专题&gt;${article.category.name }</span></p>
</div>
...
<!-- 第三个广告区 -->
<div id="bannerThree"><s:if test="adsArray.length>1">
...
<dl id="proleft">
	...
	<s:iterator value="TagCorrelationArticle">
		<dd><span><a href="${ctx}/specialcontent.action?article.id=${id}">${title}</a></span></dd>
	</s:iterator>
</dl>
...
<div id="articlelist">
					...
					<div>
					<dl>
						<dt>${member.name } , <s:date name="createDate" format="yyyy年MM月dd日" />,<s:date name="createDate" format="HH.mm.ss" /> <security:authorize ifAnyGranted="A_VIEW_ZT"><!--  <span onclick="editComment('${id}');" style="cursor: pointer;">编辑</span> --><span onclick="delComment('${id }',this);" style="cursor: pointer;">删除</span></security:authorize></dt>
						<dd>${content}</dd>
					</dl>
					</div>
					</div>
					...
					<!-- 分页验证权限 -->
					<security:authorize ifAnyGranted="A_VIEW_ZT"><input type="hidden" id="authority" value="A_VIEW" /></security:authorize>
					<div id="pagecount">
					<ul>
