...
<html xmlns="http://www.w3.org/1999/xhtml">
    <head>
		...
		<title>秀场_${showCategory.name}</title>
	...
	<div class="container">
		...
		<div id="content">
			...
			<div id="postion">
				<p>
					<span>您的位置:首页&gt;<a href="${ctx }/show?showCategory.id=56">秀场</a>&gt;${showCategory.name}</span>
				</p>
			</div>
			<div id="contentLeft">
				<div id="totop">
				...
				<div id="bannertop">
					...
					<s:if test="adsArray[5].type.name==1">
						...
						<img src="${ctx }<s:property value='adsArray[5].image'/>" height="150px" width="600px" alt="<s:property value='adsArray[5].description'/>" />
						</a>
					</s:else>
				</s:if>
				</div>
				<div id="tabshow">
					...
					<div class="news">
						<div class="newsleft">
							<a href="${ctx}/showcontent.action?article.id=${id}">
						...
						<div class="newsright">
							<h3><a href="${ctx}/showcontent.action?article.id=${id}">${title }</a></h3>
					...
					<div style="padding-left:96px">
						...
						<s:if test="page.hasPre">
							<a href="showlist.action?page.pageNo=${page.prePage}&page.orderBy=${page.orderBy}&page.order=${page.order}&showCategory.id=${showCategory.id}">上一页</a>
						</s:if>
						<ouun:pageNum totalPages="${page.totalPages}" pageNo="${page.pageNo}" url="showlist.action?page.orderBy=${page.orderBy}&page.order=${page.order}&showCategory.id=${showCategory.id}&page.pageNo=" />
						<s:if test="page.hasNext">
							<a href="showlist.action?page.pageNo=${page.nextPage}&page.orderBy=${page.orderBy}&page.order=${page.order}&showCategory.id=${showCategory.id}">下一页</a>
						</s:if>
					</div>
				...
				<div id="bannerThree">
				   <s:if test="adsArray.length>0">
						<s:if test="adsArray[6].type.name==1">
							...
							<img src="${ctx }<s:property value='adsArray[6].image'/>" height="100px" width="600px" alt="<s:property value='adsArray[6].description'/>" />
							</a>
						</s:else>
					</s:if>
				</div>
				...
				<div id="rightContent_top">
					<s:if test="adsArray.length>0">
							<s:if test="adsArray[3].type.name==1">
								...
								<img src="${ctx }<s:property value='adsArray[3].image'/>" height="120px" width="345x" alt="<s:property value='adsArray[3].description'/>" />
								</a>
							</s:else>
				  </s:if>
				</div>
				<div id="rightContent_top2">
					<s:if test="adsArray.length>0">
							<s:if test="adsArray[4].type.name==1">
								...
								<img src="${ctx }<s:property value='adsArray[4].image'/>" height="200px" width="345x" alt="<s:property value='adsArray[4].description'/>" />
								</a>
							</s:else>
				  </s:if>
				</div>
			<div class="votes">
					...
					<div id="votesweek" class="votweek">
						...
						<s:iterator value="weekVotes" status="sta">
							<s:if test="#sta.first">
								...
								<dd><a href="${ctx}/showcontent.action?article.id=${id}"><ouun:substring length="14" value="${title}"/></a></dd>
							</s:else>
						</s:iterator>
					...
					<div id="hotweek" class="votweek">
						<dl>
							...
							<s:if test="#sta.first">
								...
								<dd><a href="${ctx}/showcontent.action?article.id=${id}"  title="${title }"><ouun:substring length="14" value="${title}"/></a></dd>
							</s:else>
						</s:iterator>
						</dl>
					</div>
				...
				<div class="hotCommodity">
					...
					<p>
						...
						<a href="${ctx}/downloadcontent.action?resource.id=${id}">
