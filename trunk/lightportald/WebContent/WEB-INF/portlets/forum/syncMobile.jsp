...
{if forum.topicLists }
	...
	<td class='portlet-link' >
		...
		<a href="javascript:;" onclick="javascript:Light.globalSearch('','org.light.portlets.forum.ForumPost');"><img src='<%= request.getContextPath() %>/light/images/search.gif' height='16' width='16' align='top' title='<fmt:message key="portlet.button.search"/>'/></a>	
	</td>
	</tr>
	<tr>
	<td class='portlet-link-left' >
	...
	<table border='0' cellpadding='0' cellspacing='0' width='98%'>
		<tr>
			<td class='portlet-table-td-left' width= '60%'><fmt:message key="portlet.label.forumTopic"/></td>
			<td class='portlet-table-td-left' width= '40%' style='padding-left:20px;'><fmt:message key="portlet.label.lastPost"/></td>			
		</tr>
	{for item in forum.topicLists}
	<tr class='portlet-table-td-left'>
		<td class='portlet-item' >
			<a href='javascript:;' 
				onclick="javascript:Light.executeRender('${id}','','','categoryId=${forum.categoryId};forumId=${forum.forumId};topicId=${item.id}');">
			${item.topic}</a><label>(${item.posts} <fmt:message key="portlet.label.posts"/>)</label>
		</td>
		<td class='portlet-item'>
			<table>
			<tr>
...
<tr>
<td class='portlet-link' colspan='2'>
	{if forum.pages > 1 }
		{var pagesArray = new Array(); for(var n=1;n <= forum.pages;n++) pagesArray.push(n);}
		{for i in pagesArray }
			{if i != forum.pageId }
...
{/if}
<td class='portlet-table-td-center' width='60px'>
...
<ul style="background: url(<%= request.getContextPath() %>{if item.photoUrl }${item.photoUrl}{else}/light/images/no_pic.gif{/if}) no-repeat scroll 0pt 0pt transparent; list-style-type: none; width: 50px; height: 50px; margin: 0pt 0pt 0pt 3px; padding: 0pt; -moz-border-radius: 8px 8px 8px 8px;"><li style="font-size: 12px;"></li></ul></div></a></span> 
...
<br/>
<a href='javascript:;' onclick="javascript:showInstantMessageMember(event,'${item.postById}','${id}');" >
<img src='<%= request.getContextPath() %>/light/images/chat.gif' style='border:0px;margin:5px;' height='16' width='16' align="top" title='<fmt:message key="portlet.label.instantMessage"/>' /></a>
{/if}
<a href='javascript:;' onclick="javascript:showSendMessageMember('${id}','${item.postById}','${item.displayName}',event);" >
<img src='<%= request.getContextPath() %>/light/images/inbox.gif' style='border:0px;margin:5px;' height='16' width='16' align="top" title='<fmt:message key="portlet.label.sendMessage"/>' /></a>
</span>
</td>
<td class='portlet-table-td-left'>
...
<input type='button' onClick="javascript:Light.executeAction('${id}','','replybegin',null,'${forum.topicId}','edit',null,null);" value='<fmt:message key="portlet.button.reply"/>' class='portlet-form-button' />
