package ningbo.media.core.web.utils;
...
import ningbo.media.core.web.utils.WebConstant;
...
public class RequestUtils {
	...
	/**
	 * @Description:获取QueryString的参数，并使用URLDecoder以UTF-8格式转码。如果请求是以post方法提交的， 
	 * 				那么将通过HttpServletRequest#getParameter获取。
	 * 
	 * @see HttpServletRequest#getParamenter.
	 * @param request
	 *            web请求
	 * @param name
	 *            参数名称
	...
	public static String getQueryParam(HttpServletRequest request, String name) {
		...
		if (request.getMethod().equalsIgnoreCase(WebConstant.POST)) {
		...
		try {
			s = URLDecoder.decode(s, WebConstant.UTF8);
		} catch (UnsupportedEncodingException e) {
			log.error("encoding " + WebConstant.UTF8 + " not support?", e);
		}
	...
	/**
	 * @Function:获取访问者IP
	 * @description:本方法先从Header中获取X-Real-IP，如果不存在再从X-Forwarded-For获得第一个IP
	 * @param request
	 * @return IP Address
	 */
	...
	/**
	 * @Function:获得当的访问路径
	 * @Description:HttpServletRequest.getRequestURL+"?"+HttpServletRequest.getQueryString
	 * @param request
	 * @return
	 */
	public static String getCurrentPath(HttpServletRequest request) {
		...
		String originatingUri = helper.getOriginatingRequestUri(request);
		buffer.replace(buffer.length() - uri.length(), buffer.length(),
				originatingUri);
	...
	/**
	 * @Description:获得请求的session id，但是HttpServletRequest#getRequestedSessionId()方法有一些问题。
