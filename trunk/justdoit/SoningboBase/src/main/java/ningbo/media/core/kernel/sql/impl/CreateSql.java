...
public abstract class CreateSql {
	...
	protected static MybatisSqlModal gengerateSql(TableInfo tableInfo) {
		...
		for (ColumnInfo column : tableInfo.getColumnList()) {
			...
			selectColumn.append(Constants.TWOBLANK_SIGN).append(columnName)
					.append(",").append(Constants.WRAP_CHAR);
			if (column.isPk()) {
				...
				insertMybatis.append(Constants.TWOBLANK_SIGN).append(" #{")
						.append(clsPro).append("},")
						.append(Constants.WRAP_CHAR);
				updateColumns.append(Constants.TWOBLANK_SIGN)
						.append(columnName).append("=#{").append(clsPro)
						.append("},").append(Constants.WRAP_CHAR);
			}
		}
		mybatisSqlModal.setPrimaryKeys(primaryKeys);
		mybatisSqlModal.setInsertColumn(StringUtil.deleteLastChar(
				insertColumns, ","));
		mybatisSqlModal.setSelectColumn(StringUtil.deleteLastChar(selectColumn,
				","));
		mybatisSqlModal.setUpdateColumn(StringUtil.deleteLastChar(
				updateColumns, ","));
		mybatisSqlModal.setInsertMybatis(StringUtil.deleteLastChar(
				insertMybatis, ","));
	 ...
	 * 
	 * @param mybatisSqlModal
	 *            mybatis信息
	 * @param tableName
	 *            数据库表名
	 * @return insert脚本
	 */
	protected static String generateInsertSql(MybatisSqlModal mybatisSqlModal,
			String tableName) {
		...
		insertSql.append("INSERT INTO ").append(tableName).append("(")
				...
				.append("VALUES (").append(Constants.WRAP_CHAR)
	 ...
	 * @return update脚本
	 */
	protected static String generateUpdateSql(MybatisSqlModal mybatisSqlModal,
			String tableName) {
		StringBuilder updateSql = new StringBuilder();
		updateSql.append("UPDATE ").append(tableName)
				.append(Constants.WRAP_CHAR).append(Constants.BLANK_SIGN)
				.append("SET").append(Constants.WRAP_CHAR)
				.append(mybatisSqlModal.getUpdateColumn())
				.append(Constants.WRAP_CHAR).append(Constants.BLANK_SIGN)
				.append(Constants.WHERE_KEY).append(Constants.WRAP_CHAR);
		for (String primary : mybatisSqlModal.getPrimaryKeys()) {
			updateSql.append(Constants.TWOBLANK_SIGN)
					...
					.append(StringUtil.columnToPropertis(primary)).append("}")
					.append(Constants.WRAP_CHAR).append(Constants.BLANK_SIGN)
	 ...
	 * @return 查询单个信息的脚本
	 */
	protected static String generateSelectOneSql(
			MybatisSqlModal mybatisSqlModal, String tableName) {
		StringBuilder selectOneSql = new StringBuilder();
		selectOneSql.append(Constants.SELECT_KEY).append(Constants.WRAP_CHAR)
	 ...
	 * @return 求总记录的sql脚本
	 */
	protected static String generateCountSql(MybatisSqlModal mybatisSqlModal,
			String tableName) {
		List<String> primarys = mybatisSqlModal.getPrimaryKeys();
		StringBuilder countSql = new StringBuilder();
		countSql.append(Constants.SELECT_KEY);
		if (primarys.size() > 0) {
			countSql.append("COUNT(").append(primarys.get(0)).append(")");
		} else {
			countSql.append("COUNT(*)");
		}
		countSql.append(Constants.WRAP_CHAR).append(Constants.BLANK_SIGN)
				.append(Constants.FROM_KEY).append(tableName);
	 ...
	 * @return delete脚本
	 */
	protected static String generateDeleteSql(MybatisSqlModal mybatisSqlModal,
			String tableName) {
		StringBuilder deleteSql = new StringBuilder();
		deleteSql.append(Constants.DELETE_KEY).append(tableName)
				.append(Constants.WRAP_CHAR);
		for (String primary : mybatisSqlModal.getPrimaryKeys()) {
	 ...
	 * @return 分页sql的Where
	 *         <p>
	 *         例如：mysql的生成：limit start and end
	 *         </p>
	 */
	protected abstract String pageParamSql();

	/**
	 * 产生获取刚写入数据库信息表的主键值的相关sql。 暂时不实现
	 * 
	 * @return 写入获取主键相关sql
	 */
	 ...
	 * @return 查询脚本
	 */
	protected abstract String pageSelectSql(MybatisSqlModal mybatisSqlModal,
			String tableName);
