...
public class InvoicetempleteTAction extends ActionSupport {
	...
	public String addInvoiceTemplete() {
			...
			if (this.getInvoicetempleteTServiceImpl().addInvoiceTemplete(it) > 0) {
				return SUCCESS;
			} else {
				return INPUT;
			}
	...
	public String findAllInvoicetempleteT() {
		...
		int lineSize = rp;
		List<InvoicetempleteT> list = this.getInvoicetempleteTServiceImpl().findAllInvoicetempleteT(currentPage, lineSize);
		if (list != null) {
			total = this.getInvoicetempleteTServiceImpl().countfindAllInvoicetempleteT();
			rows.clear();
	...
	public String UpdateInvoicetempleteT() {
			...
			it.setKindeditorCode(this.getInvoiceCode());
			int i = this.getInvoicetempleteTServiceImpl().updateInvoicetempleteT(it);
			return SUCCESS;
	...
	public String DelInvoiceTemplete() {
		if (Validate.StrNotNull(this.getInvoicetempleteid())) {
			...
			if (this.getInvoicetempleteTServiceImpl().delInvoiceTemplete(array) > 0) {
				return "json";
			}
			return "json";
		}
	...
	public String findInvoicetempleteById() {
		if (Validate.StrNotNull(this.getInvoicetempleteid())) {
			beanlist = this.getInvoicetempleteTServiceImpl().findInvoicetempleteById(this.getInvoicetempleteid().trim());
			if (beanlist != null) {
				return "json";
			}
		}
