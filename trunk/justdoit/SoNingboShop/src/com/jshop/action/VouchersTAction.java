...
public class VouchersTAction extends ActionSupport {
	...
	public String addVoucherst() throws ParseException {
		...
		if (adminid != null) {
			List<VouchersT> vlist = this.getVouchersTServiceImpl().findVoucherstByName(this.getVouchersname().trim());
			if (vlist.size() > 0) {
				return "json";
			}
			...
			if (this.getVouchersTServiceImpl().addVoucherst(vt) > 0) {
	...
	public String findAllVoucherst() {
		...
		int lineSize = rp;
		List<VouchersT> list = this.getVouchersTServiceImpl().findAllVoucherst(currentPage, lineSize);
		if (list != null) {
			total = this.getVouchersTServiceImpl().countfindAllVoucherst();
			rows.clear();
	...
	public String DelVoucherst() {
		if (this.getVouchersid() != null) {
			...
			if (this.getVouchersTServiceImpl().delVoucherst(array) > 0) {
				return "json";
			}
			return "json";
		}
