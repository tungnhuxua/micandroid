...
public class BrandTAction extends ActionSupport {
	@Resource(name = "brandTServiceImpl")
	private BrandTServiceImpl brandTServiceImpl;
	...
	public BrandTServiceImpl getBrandTServiceImpl() {
		return brandTServiceImpl;
	}

	public void setBrandTServiceImpl(BrandTServiceImpl brandTServiceImpl) {
		this.brandTServiceImpl = brandTServiceImpl;
	}
	...
	public String addBrandt() {
		...
		if (this.getBrandTServiceImpl().addBrandt(bt) > 0) {
	...
	public String findAllBrandt() {
		...
		int lineSize = rp;
		total = this.getBrandTServiceImpl().countfindAllBrandt(BaseTools.adminCreateId());
		if (Validate.StrNotNull(sortname) && Validate.StrNotNull(sortorder)) {
			...
			List<BrandT> bt = this.getBrandTServiceImpl().sortAllBrandt(currentPage, lineSize, BaseTools.adminCreateId(), queryString);
			if (bt != null) {
	...
	public String UpdateBrandt() {
		...
		bt.setSort(Integer.parseInt(this.getSort().trim()));
		bt.setUrl(this.getUrl().trim());
		this.getBrandTServiceImpl().updateBrandt(bt);
		this.setSucflag(true);
		return "json";
	}
	...
	public String findBrandById() {
		if (Validate.StrNotNull(this.getBrandid())) {
			bean = this.getBrandTServiceImpl().findBrandById(this.getBrandid().trim());
			if (bean != null) {
				return "json";
			}
		}
	...
	public String DelBrandt() {
		if (Validate.StrNotNull(this.getBrandid())) {
			...
			if (this.getBrandTServiceImpl().delBrandt(strs, BaseTools.adminCreateId()) > 0) {
				return "json";
			}
			return "json";
		}
	...
	public String findAllBrandtjson() {
		this.setBrandjson("");
		this.brand = this.getBrandTServiceImpl().findAllBrandtjson(BaseTools.adminCreateId());
		if (this.brand != null) {
