...
public class ExpresstempleteTAction extends ActionSupport {
	...
	public String addExpresstemplete() {
			...
			if (this.getExpresstempleteTServiceImpl().addExpresstemplete(et) > 0) {
				return SUCCESS;
			}else{
				return INPUT;
			}
	...
	public String findAllExpresstempleteT() {
		...
		int lineSize = rp;
		List<ExpresstempleteT> list = this.getExpresstempleteTServiceImpl().findAllExpresstempleteT(currentPage, lineSize);
		if (list != null) {
			total = this.getExpresstempleteTServiceImpl().countfindAllExpresstempleteT();
			rows.clear();
	...
	public String UpdateExpresstemplete() {
			...
			int i = this.getExpresstempleteTServiceImpl().updateExpresstemplete(et);
			return SUCCESS;
	}
	...
	public String findExpresstempleteBytempleteid() {
		if (Validate.StrNotNull(this.getExpresstempleteid())) {
			beanlist = this.getExpresstempleteTServiceImpl().findExpresstempleteBytempleteid(this.getExpresstempleteid().trim());
			if (beanlist != null) {
				return "json";
			}
		}
	...
	public String DelExpresstemplete() {
		if (Validate.StrNotNull(this.getExpresstempleteid())) {
			...
			if (this.getExpresstempleteTServiceImpl().delExpresstemplete(strs) > 0) {
				return "json";
			}
			return "json";
		}
