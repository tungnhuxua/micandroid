...
public class PaymentMAction extends ActionSupport {
	...
	public String addPayment() {
		...
		if (this.getPaymentMServiceImpl().addPayment(pm) > 0) {
			return SUCCESS;
		}
		return INPUT;
	...
	public String findAllPayment() {
		...
		int lineSize = rp;
		List<PaymentM> list = this.getPaymentMServiceImpl().findAllPayment(currentPage, lineSize);
		if (list != null) {
			total = this.getPaymentMServiceImpl().countfindAllPayment();
			rows.clear();
	...
	public String OpenPayment() {
		...
		if (Validate.StrNotNull(this.getPaymentid())) {
			...
			if (this.getPaymentMServiceImpl().openPayment(strs) > 0) {
				return "json";
			}
			return "json";
		}
	...
	public String ClosePayment() {
		...
		if (Validate.StrNotNull(this.getPaymentid())) {
			String[] strs = this.getPaymentid().trim().split(",");
			if (this.getPaymentMServiceImpl().closePayment(strs) > 0) {
				return "json";
			}
			return "json";
		}
	...
	public String findPaymentbyId() {
		...
		if (Validate.StrNotNull(this.getPaymentid())) {
			beanlist = this.getPaymentMServiceImpl().findPaymentbyId(this.getPaymentid().trim());
			if (beanlist != null) {
				return "json";
			}
		}
