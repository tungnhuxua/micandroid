...
public class LogisticsbusinessareaTAction extends ActionSupport {
	...
	public String addLogisticsbusinessarea() {
		...
		lba.setState("1");
		if (this.getLogisticsbusinessareaTServiceImpl().addLogisticsbusinessarea(lba) > 0) {
			return SUCCESS;
		}
		return INPUT;
	...
	public String findAllLogisticsbusinessareaT() {
		...
		int lineSize = rp;
		List<LogisticsbusinessareaT> list = this.getLogisticsbusinessareaTServiceImpl().findAllLogisticsbusinessareaT(currentPage, lineSize);
		if (list != null) {
			total = this.getLogisticsbusinessareaTServiceImpl().countfindAllLogisticsbusinessareaT();
			rows.clear();
	...
	public String UpdateLogisticsbusinessarea() {
		...
		lba.setSendarea(this.getSendarea().trim());
		lba.setState("1");
		this.getLogisticsbusinessareaTServiceImpl().updateLogisticsbusinessarea(lba);
		return "json";
	...
	public String findLogisticsbusinessareaTById() {
		if (Validate.StrNotNull(this.getLogbusareaid())) {
			beanlist = this.getLogisticsbusinessareaTServiceImpl().findLogisticsbusinessareaTById(this.getLogbusareaid().trim());
			if (beanlist != null) {
				return "json";
			}
		}
	...
	public String DelLogisticsbusinessarea() {
		if (Validate.StrNotNull(this.getLogbusareaid())) {
			...
			if (this.getLogisticsbusinessareaTServiceImpl().delLogisticsbusinessarea(strs) > 0) {
				return "json";
			}
			return "json";
		}
	...
	public String UpdateLogisticsbusinessareaBystate() {
		if (Validate.StrNotNull(this.getLogbusareaid()) && Validate.StrNotNull(this.getState())) {
			...
			this.getLogisticsbusinessareaTServiceImpl().updateLogisticsbusinessareaBystate(array, this.getState().trim());
			return "json";
		}
		return "json";
	}
