...
import com.jshop.service.impl.FavoriteTServiceImpl;
...
@Controller("myFavoriteAction")
public class MyFavoriteAction extends ActionSupport {

	@Resource(name="serial")
	private Serial serial;
	@Resource(name="favoriteTServiceImpl")
	private FavoriteTServiceImpl favoriteTServiceImpl;
	...
	@JSON(serialize=false)
	public FavoriteTServiceImpl getFavoriteTServiceImpl() {
		return favoriteTServiceImpl;
	}
	public void setFavoriteTServiceImpl(FavoriteTServiceImpl favoriteTServiceImpl) {
		this.favoriteTServiceImpl = favoriteTServiceImpl;
	}
	...
	public String addFavorite(){
		...
		if(user!=null){
			...
			if(this.getFavoriteTServiceImpl().addFavorite(f)>0){
	...
	public String GetMyFavoriteByUserId(){
		...
		if(user!=null){
			...
			List<FavoriteT>list=this.getFavoriteTServiceImpl().findAllFavoriteByUserid(user.getUserid(), currentPage, lineSize);
			try {
				int allRecorders=this.getFavoriteTServiceImpl().countfindAllFavoriteByUserid(user.getUserid());
			} catch (Exception e) {
				e.printStackTrace();
			}
			if(list!=null){
	...
	public String DelMyFavoriteByid(){
		...
		if(user!=null){
			if(Validate.StrNotNull(this.getFavoriteid())){
				...
				if(this.getFavoriteTServiceImpl().delFavorite(strs)>0){
					return SUCCESS;
				}else{
					return INPUT;
				}
