...
public class ZoneDao extends HibernateDaoSupport implements IZoneDao {
	...
	public Collection<Zone> findByCountryIdAndByLanguageId(int countryId, int languageId) {
		...
		try {
			...
			List l = super
					.getSession()
					.createQuery(
							"select z from Zone z left join fetch z.Descriptions s where s.id.languageId=:lId and z.zoneCountryId=:cId")
