...
import com.salesmanager.core.entity.reference.StateProvinceVO;
...
public class ReferenceService {
	...
	@Transactional
	public Collection<Country> getCountriesByLanguageId(int languageId)
			throws Exception {
		return countryDao.findByLanguageId(languageId);
	}
	...
	@Transactional
	public Collection<StateProvinceVO> getStateProvinceByCountryIdAndByLanguageId(int countryId, int languageId) throws Exception {
		Collection zones = zoneDao.findByCountryIdAndByLanguageId(countryId, languageId);
		...
		for(Object o : zones) {
			...
			Zone z = (Zone)o;
			StateProvinceVO p = new StateProvinceVO();
			p.setZoneId(z.getZoneId());
			p.setZoneName(z.getName());
			l.add(p);
	...
	/**
	 * Get a DynamicLabel for a given merchant id, a seo url and a Locale
	 * @param merchantId
	 * @param url
	 * @param locale
	 * @return
	...
	/**
	 * Get a DynamicLabel for a given merchant id and a seo url
	 * @param merchantId
	 * @param url
	 * @param locale
	 * @return
	 */
	@Transactional
	public DynamicLabel getDynamicLabelByMerchantIdAndSeUrl(
			int merchantId, String url) {
		return dynamicLabelDao.findByMerchantIdAndSeUrl(
				merchantId, url);
	}
