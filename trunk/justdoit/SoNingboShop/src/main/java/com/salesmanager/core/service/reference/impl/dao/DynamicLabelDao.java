...
public class DynamicLabelDao extends HibernateDaoSupport implements
	...
	public DynamicLabel findByMerchantIdAndSeUrl(int merchantId,
			String url) {
		...
		try {
			...
			List l = super
					.getSession()
					.createQuery(
							"select d from DynamicLabel d left join fetch d.descriptions s where d.merchantId=:mId and s.seUrl=:sUrl")
					.setInteger("mId", merchantId)
					.setString("sUrl", url)
					.list();
