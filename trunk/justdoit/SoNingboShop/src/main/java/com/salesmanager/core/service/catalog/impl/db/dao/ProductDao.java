...
/*	public SearchProductResponse findProductsByDescription(
	...
	public Product findProductByMerchantIdAndSeoURL(int merchantId,
			String seUrl) {

		try {

			String query = "select p from Product p left join fetch p.descriptions s left join fetch p.specials y left join fetch p.prices r left join fetch r.priceDescriptions rp left join fetch r.special x where p.merchantId=:mId and s.seUrl=:sText";
