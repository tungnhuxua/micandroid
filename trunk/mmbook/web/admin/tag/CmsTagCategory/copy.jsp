...
<html>
<head>
<title>复制栏目标签页面</title>
<script type="text/javascript">

Ext.ux.cmstagcategory.copy<%=u_id%>=Ext.extend(function(){
...
var fun_settest=function(textfield,value){
		frm_copy_cmstagcategory<%=u_id%>.form.findField(textfield).setValue(value);
}
 
var getId_ = function(id){
     frm_copy_cmstagcategory<%=u_id%>.form.findField("categoryId").setValue(id);
};
/**模板ID*/
...
comboBoxCheckTree.on('change', function(box, newvar, oldvar){
  frm_copy_cmstagcategory<%=u_id%>.form.findField('categoryId').setValue(comboBoxCheckTree.getValue()); 
});
			         ...
			         items: [
						  {
								...
								listeners:{"select":function(){//根据业务添加监听事件
								     ...
								     frm_copy_cmstagcategory<%=u_id%>.form.findField('templetId').setValue(frm_copy_cmstagcategory<%=u_id%>.form.findField('templetName').getValue());
								    }
								  }
						  }
	                    ]
...
var frm_copy_cmstagcategory<%=u_id%> = new Ext.FormPanel({
        layout: 'fit',
        labelAlign: 'left',
        frame:true,
        title: '复制',
        ...
        items: [field_copy_base<%=u_id%>,field_copy_dataDisplay<%=u_id%>,field_copy_dataTransfer<%=u_id%>,field_copy_datatemplet<%=u_id%>],
          ...
          buttons: [{
  	            text:'复制',
  	            handler:function(){
  				if (frm_copy_cmstagcategory<%=u_id%>.form.isValid() == false){
  	    		    return;
  	  		    }
  	  		    Ext.MessageBox.confirm('确认操作','确定复制栏目标签页面?',function(btn){
  	            if (btn == 'yes'){
  	  		    frm_copy_cmstagcategory<%=u_id%>.form.submit({
  	   		    ...
  	   		    success:function(form,action){
			          ...
			          //fun_copnetab('<%//=u_id%>','栏目标签管理','admin/tag/CmsTagCategory/index.jsp?u=<%=u_id%>&v=<%=v_id%>','icon-nav-p1');
	   		          }});
  	 		       },
  	 		     scope:this,
...
var panel_copy_cmstagcategory<%=u_id%> = new Ext.Panel({
  		renderTo:'copy_cmstagcategorydiv<%=u_id%>',
  		width:Ext.get("copy_cmstagcategorydiv<%=u_id%>").getWidth(),
  		height:Ext.get("copy_cmstagcategorydiv<%=u_id%>").getHeight(),
  		border : false,
  		layout : 'border',
        items:[frm_copy_cmstagcategory<%=u_id%>]
});

fun_field_fit();
if (record<%=u_id%>) {
      frm_copy_cmstagcategory<%=u_id%>.form.loadRecord(record<%=u_id%>);
}else{
      Ext.Msg.show({title: '提示', msg: '请选择需复制的记录!',icon: Ext.Msg.INFO,minWidth: 200,buttons: Ext.Msg.OK});
}

panel<%=u_id%>.on('resize',function(){
    if(Ext.get("copy_cmstagcategorydiv<%=u_id%>")){
        var p =panel<%=u_id%>.getActiveTab().getId();
        if(p!='copy_CmsTagCategory<%=u_id%>'){
             panel<%=u_id%>.setActiveTab('copy_CmsTagCategory<%=u_id%>');
             panel_copy_cmstagcategory<%=u_id%>.setWidth(Ext.getCmp('center-tab-panel').getInnerWidth()-2);
             panel_copy_cmstagcategory<%=u_id%>.setHeight(Ext.getCmp('center-tab-panel').getInnerHeight()-2);
             fun_field_fit();
...
new Ext.ux.cmstagcategory.copy<%=u_id%>();
</script>
</head>
<body>
<div id="copy_cmstagcategorydiv<%=u_id%>" style="width:100%;height:100%;"></div>
</body>
</html>