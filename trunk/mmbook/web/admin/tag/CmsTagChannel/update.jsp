...
<html>
<head>
<title>修改频道标签页面</title>
<script type="text/javascript">

Ext.ux.cmstagchannels.update<%=u_id%>=Ext.extend(function(){
...
var fun_settest=function(textfield,value){
		frm_update_cmstagchannels<%=u_id%>.form.findField(textfield).setValue(value);
}
 
var getId_ = function(id){
     frm_update_cmstagchannels<%=u_id%>.form.findField("categoryId").setValue(id);
};
            ...
            items:[{
                	...
                	items: [{
							  ...
							  listeners:{"select":function(){//根据业务添加监听事件
								         frm_update_cmstagchannels<%=u_id%>.form.findField('categoryId').setValue(frm_update_cmstagchannels<%=u_id%>.form.findField('channelsids').getValue()); 
							            }
	                          }
                        }]
	           }]
            ...
            items:[{
			         ...
			         items: [{
								...
								listeners:{"select":function(){//根据业务添加监听事件
								     ...
								     frm_update_cmstagchannels<%=u_id%>.form.findField('templetId').setValue(frm_update_cmstagchannels<%=u_id%>.form.findField('templetName').getValue());
								    }
							 }
						  }
	                    ]
...
var frm_update_cmstagchannels<%=u_id%> = new Ext.FormPanel({
        layout: 'fit',
        labelAlign: 'left',
        frame:true,
        title: '修改',
        ...
        items: [field_update_base<%=u_id%>,field_update_dataDisplay<%=u_id%>,field_update_dataTransfer<%=u_id%>,field_update_datatemplet<%=u_id%>],
          ...
          buttons: [{
  	            text:'修改',
  	            handler:function(){
  				if (frm_update_cmstagchannels<%=u_id%>.form.isValid() == false){
  	    		    return;
  	  		    }
  	  		    Ext.MessageBox.confirm('确认操作','确定修改频道标签页面?',function(btn){
  	            if (btn == 'yes'){
  	  		    frm_update_cmstagchannels<%=u_id%>.form.submit({
  	   		    ...
  	   		    success:function(form,action){
			          ...
			          if (!(typeof(pag_cmstagchannels<%=u_id%>) == "undefined")) {
...
var panel_update_cmstagchannels<%=u_id%> = new Ext.Panel({
  		renderTo:'update_cmstagchannelsdiv<%=u_id%>',
  		width:Ext.get("update_cmstagchannelsdiv<%=u_id%>").getWidth(),
  		height:Ext.get("update_cmstagchannelsdiv<%=u_id%>").getHeight(),
  		border : false,
  		layout : 'border',
        items:[frm_update_cmstagchannels<%=u_id%>]
});

fun_field_fit();
if (record<%=u_id%>) {
      frm_update_cmstagchannels<%=u_id%>.form.loadRecord(record<%=u_id%>);
}else{
      Ext.Msg.show({title: '提示', msg: '请选择需修改的记录!',icon: Ext.Msg.INFO,minWidth: 200,buttons: Ext.Msg.OK});
}

panel<%=u_id%>.on('resize',function(){
    if(Ext.get("update_cmstagchannelsdiv<%=u_id%>")){
        var p =panel<%=u_id%>.getActiveTab().getId();
        if(p!='update_cmstagchannels<%=u_id%>'){
             panel<%=u_id%>.setActiveTab('update_cmstagchannels<%=u_id%>');
             panel_update_cmstagchannels<%=u_id%>.setWidth(Ext.getCmp('center-tab-panel').getInnerWidth()-2);
             panel_update_cmstagchannels<%=u_id%>.setHeight(Ext.getCmp('center-tab-panel').getInnerHeight()-2);
             fun_field_fit();
...
new Ext.ux.cmstagchannels.update<%=u_id%>();
</script>
</head>
<body>
<div id="update_cmstagchannelsdiv<%=u_id%>" style="width:100%;height:100%;"></div>
</body>
</html>