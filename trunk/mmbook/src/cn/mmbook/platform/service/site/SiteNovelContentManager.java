...
/**
 * <p> SiteNovelContent server 接口<br> 
 * <p>   <br>
 * @author admin,
...
public interface SiteNovelContentManager {
	/**按ID 查询对象*/
	SiteNovelContent getById(java.lang.Integer id);
	/**查询所有数据*/
	List findAll();
	/**查询数据返回LIST*/
	List getList(SiteNovelContent u);
	/**保存数据 由BaseManager类实现*/
	void save(SiteNovelContent u);
	/**修改数据由BaseManager类实现*/
	void update(SiteNovelContent u);
	/**删除数据由BaseManager类实现*/
