			...
			if (null == res) {
				...
				if (res.Status == 'OK') {
					...
					for ( var i = 0, j = lists.length; i < j; i++) {
						...
						var t = item.Phones[0].PhoneNumber;
		...
		if (json.length > 0) {
			for ( var i = 0, j = json.length; i < j; i++) {
				...
				$(".c_details_content").append(
						"<li class='li_default'><div class='info_area p_info'>"
								+ temp.telephone
								+ "</div><div class='info_area e_info'>"
								+ temp.uemail
								+ "</div><div class='info_area n_info'>"
								+ temp.companyName + "</div></li>");
