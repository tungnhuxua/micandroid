...
public class ComboDefine extends BusinessObject {
	...
	public String configJson(String funid, String fieldCode) {
		...
		//查找选择控件的功能ID
		String sqlCtl = "select fun_id, layout_page from funall_control where control_code = ?";
		DaoParam paramCtl = _dao.createParam(sqlCtl);
		paramCtl.setDsName(DefineName.DESIGN_NAME);
		paramCtl.addStringValue(ctlCode);
		Map<String,String> mpCtl = _dao.queryMap(paramCtl);
		if (mpCtl.isEmpty()) {
			_log.showWarn("not find combowin control: control_code={0}!", 
					ctlCode);
			return "null";
		}
		...
		if (mpExt.isEmpty()) {
			_log.showWarn("not find combowin type field ext info: funid={0} fieldcode={1}!", 
					funid, fieldCode);
			//return "null";
			mpExt.put("is_same", "1");
