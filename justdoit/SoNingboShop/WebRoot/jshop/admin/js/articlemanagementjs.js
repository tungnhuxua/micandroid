			...
			if ($('.trSelected', grid).length == 1) {
				jConfirm('确定编辑此项吗?', '信息提示', function(r) {
					if (r) {
						...
						window.location.href = "addarticle.jsp?session="+session+"#articleid=" + str;
						return;
					}
				});
			} else {
...
$(function(){
	$('#submit').click(function(){
		...
		$.post("addArticleT.action",{"articleCategoryTid":articleCategoryTid,"articleCategoryName":articleCategoryName,"metaKeywords":metaKeywords,"metaDes":metaDes,"title":title,"contentvalue":contentvalue,"author":author,"ispublication":ispublication,"isrecommend":isrecommend,"istop":istop,"pageCount":pageCount},function(data){
			if(data.sucflag){
...
$(function(){
	...
	$('#modify').click(function(){
		...
		$.post("updateArticleT.action",{"articleCategoryTid":articleCategoryTid,"articleid":articleid,"articleCategoryName":articleCategoryName,"metaKeywords":metaKeywords,"metaDes":metaDes,"title":title,"contentvalue":contentvalue,"author":author,"ispublication":ispublication,"isrecommend":isrecommend,"istop":istop,"pageCount":pageCount,"readcount":readcount},function(data){
			if(data.sucflag){
				window.location.href="articlemanagement.jsp?session="+session+"#pagecontent";
			}else{
				jAlert('文章更新失败', '信息提示');
