...
public class CategoryDao extends HibernateDaoSupport implements ICategoryDao {
	...
	public Collection<Category> findByCategoryIdsAndMerchantId(Collection<Long> categoryIds, int merchantId) {
		...
		try {
			...
			DetachedCriteria crit = DetachedCriteria.forClass(Category.class);
			crit.add(Expression.in("categoryId", categoryIds));
			crit.add(Expression.eq("merchantId", merchantId));
			Collection result = this.getHibernateTemplate()
					.findByCriteria(crit);
	...
	public Collection<Category> findByMerchantIdAndLineage(int merchantId,
		...
		try {
			List list = super
					.getSession()
					.createQuery(
							"select c from Category c left join fetch c.descriptions s join fetch c.parent where c.merchantId=:mId and c.lineage like :lin order by c.depth, c.sortOrder")
...
/*	public Collection<Category> findBySubCategories(int merchantId,
			List<Long> subs, int languageId) {
		try {
			...
			for(Long l : subs) {
				...
				lineage.append("c.lineage like '%/").append(l).append("/%'");
				if(i<subs.size()) {
					lineage.append(" or ");
				}
				i++;
			...
			List list = super
					.getSession()
					.createQuery(new StringBuilder().append("select c from Category c left join fetch c.descriptions s where c.merchantId=:mId and ")
							.append(lineage.toString())
							.append(" order by c.lineage, c.sortOrder").toString())
...
/*	public List<Category> findByMerchantIdAndLanguage(int merchantId,
	...
	public Category findCategoryByMerchantIdAndSeoURL(int merchantId,
			String seoUrl) {
		...
		try {
			...
			List list = super
					.getSession()
					.createQuery(
							"select c from Category c left join fetch c.descriptions s where c.merchantId=:mId and s.seUrl=:sText order by c.sortOrder")
					.setInteger("mId", merchantId).setString("sText", seoUrl).list();
	...
	public Category getCategoryByLang(long categoryId, int languageId) {
		
		try {

			
			Category c = (Category)super
				.getSession()
				.createQuery(
					"select c from Category c left join fetch c.descriptions s where c.categoryId=:cId and s.id.languageId=:lId")
					.setLong("cId", categoryId)
