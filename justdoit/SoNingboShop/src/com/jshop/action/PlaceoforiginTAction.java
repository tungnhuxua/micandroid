...
public class PlaceoforiginTAction extends ActionSupport {
	@Resource(name="placeoforiginTServiceImpl")
	private PlaceoforiginTServiceImpl placeoforiginTServiceImpl;
	...
	public PlaceoforiginTServiceImpl getPlaceoforiginTServiceImpl() {
		return placeoforiginTServiceImpl;
	}

	public void setPlaceoforiginTServiceImpl(PlaceoforiginTServiceImpl placeoforiginTServiceImpl) {
		this.placeoforiginTServiceImpl = placeoforiginTServiceImpl;
	}
	@JSON(serialize = false)
	...
	public String addPlaceoforigint() {
		...
		if(!this.isSlogin()){
			...
			if (this.getPlaceoforiginTServiceImpl().addPlaceoforigint(p) > 0) {
				return SUCCESS;
			}
			return INPUT;
		}else{
	...
	public String findAllPlaceoforigint() {
		...
		int lineSize = rp;
		List<PlaceoforiginT> pt = this.getPlaceoforiginTServiceImpl().findAllPlaceoforigint(currentPage, lineSize);
		if (pt != null) {
			total = this.getPlaceoforiginTServiceImpl().countfindAllPlaceoforigint();
			rows.clear();
	...
	public String UpdatePlaceoforigint() {
		...
		if(!this.isSlogin()){
			...
			p.setCreatetime(BaseTools.systemtime());
			p.setCreatorid(BaseTools.adminCreateId());
			this.getPlaceoforiginTServiceImpl().updatePlaceoforigint(p);
			return SUCCESS;
		}else{
			return INPUT;
		}
	...
	public String findPlaceoforigintById() {
		...
		if(!this.isSlogin()){
			if (Validate.StrNotNull(this.getPlaceid())) {
				beanlist = this.getPlaceoforiginTServiceImpl().findPlaceoforigintById(this.getPlaceid().trim());
				if (beanlist != null) {
					return "json";
				}
				return "json";
	...
	public String DelPlaceoforigint() {
		...
		if(!this.isSlogin()){
			if (Validate.StrNotNull(this.getPlaceid())) {
				...
				if (this.getPlaceoforiginTServiceImpl().delPlaceoforigint(strs) > 0) {
					return "json";
				}
				return "json";
			}
	...
	public String findAllPlaceoforigintjson() {
		...
		this.placeoforigin = this.getPlaceoforiginTServiceImpl().findAllPlaceoforigintjson();
