...
public class GradeTAction extends ActionSupport {
	@Resource(name="gradeTServiceImpl")
	private GradeTServiceImpl gradeTServiceImpl;
	...
	public GradeTServiceImpl getGradeTServiceImpl() {
		return gradeTServiceImpl;
	}

	public void setGradeTServiceImpl(GradeTServiceImpl gradeTServiceImpl) {
		this.gradeTServiceImpl = gradeTServiceImpl;
	}
	@JSON(serialize = false)
	...
	public String addGradet() {
			...
			List<GradeT> glist = this.getGradeTServiceImpl().findAllGradeByValue(this.getGradevalue());
			if (glist.size() > 0) {
			...
			if (this.getGradeTServiceImpl().addGradet(gt) > 0) {
				return SUCCESS;
			}
			return INPUT;
	}
	...
	public String findAllGrade() {
		...
		int lineSize = rp;
		List<GradeT> list = this.getGradeTServiceImpl().findAllGrade(currentPage, lineSize);
		if (list != null) {
			total = this.getGradeTServiceImpl().countfindAllGrade();
			rows.clear();
	...
	public String findGradeById() {
		if (Validate.StrNotNull(this.getGradeid())) {
			beanlist = this.getGradeTServiceImpl().findGradeById(this.getGradeid().trim());
			if (beanlist != null) {
				return "json";
			}
		}
	...
	public String UpdateGradeById() {
			...
			gt.setCreatetime(BaseTools.systemtime());
			gt.setCreatorid(BaseTools.adminCreateId());
			this.getGradeTServiceImpl().updateGradeById(gt);
			return "json";
