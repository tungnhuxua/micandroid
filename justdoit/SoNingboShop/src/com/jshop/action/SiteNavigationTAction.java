...
public class SiteNavigationTAction extends ActionSupport {
	...
	public void findDefaultAllSiteNavigation() {
		...
		int lineSize = rp;
		total = this.getSiteNavigationTServiceImpl().countfindAllSiteNavigationT(BaseTools.adminCreateId());
		if (Validate.StrNotNull(sortname) && Validate.StrNotNull(sortorder)) {
			...
			List<SiteNavigationT> list = this.getSiteNavigationTServiceImpl().sortAllSiteNavigationT(currentPage, lineSize, BaseTools.adminCreateId(), queryString);
			if (list != null) {
	...
	public String addSiteNavigationT() {
		...
		if (this.getSiteNavigationTServiceImpl().addSiteNavigationT(sn) > 0) {
			//这里需要对应模板生成静态页面
	...
	public String findSiteNavigationBysnid() {
		...
		bean = this.getSiteNavigationTServiceImpl().findSiteNavigationBysnid(this.getSnid());
		if (bean != null) {
			this.setSucflag(true);
			return "json";
		}
	...
	public String updateSiteNavigationT() {
		...
		sn.setSort(Integer.parseInt(this.getSort()));
		sn.setSign(this.getSign());
		if (this.getSiteNavigationTServiceImpl().updateSiteNavigationT(sn) > 0) {
			//更新模板页面数据
			this.setSucflag(true);
			return "json";
		}
	...
	public String delSiteNavigationT() {
		...
		if (Validate.StrNotNull(this.getSnid())) {
			...
			this.getSiteNavigationTServiceImpl().delSiteNavigationT(strs);
			this.setSucflag(true);
			return "json";
		}
		return "json";
