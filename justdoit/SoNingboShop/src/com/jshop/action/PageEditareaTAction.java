...
public class PageEditareaTAction extends ActionSupport {
	...
	public String addPageEditareaT() {
		...
		if (this.getPageEditareaTServiceImpl().addPageEditareaT(pea) > 0) {
	...
	public void findDefaultAllPageareaT() {
		...
		int lineSize = rp;
		total = this.getPageEditareaTServiceImpl().countfindAllPageEditareaT(BaseTools.adminCreateId());
		List<PageEditareaT> list = this.getPageEditareaTServiceImpl().findAllPageEditareaT(currentPage, lineSize, BaseTools.adminCreateId());
		if (list != null) {
	...
	public String UpdatePageEditareaTBystate() {
		...
		if (Validate.StrNotNull(this.getPageeditareaid())) {
			...
			int i = this.getPageEditareaTServiceImpl().updatePageEditareaTBystate(array, this.getState(), BaseTools.adminCreateId());
			this.setSucflag(true);
			return "json";
		}
		this.setSucflag(false);
	...
	public String UpdatePageEditareaT() {
		...
		pea.setCreatorid(BaseTools.adminCreateId());
		pea.setCreatetime(BaseTools.systemtime());
		if (this.getPageEditareaTServiceImpl().updatePageEditareaT(pea) > 0) {
			;
			this.setSucflag(true);
			return "json";
		} else {
	...
	public String findPageEditareaById() {
		...
		if (Validate.StrNotNull(this.getPageeditareaid())) {
			bean = this.getPageEditareaTServiceImpl().findPageEditareaById(this.getPageeditareaid());
			if (bean != null) {
				this.setSucflag(true);
				return "json";
			}
	...
	public String delPageEditareaTBypageeditareaid() {
		if (Validate.StrNotNull(this.getPageeditareaid())) {
			...
			if (this.getPageEditareaTServiceImpl().delPageEditareaTBypageeditareaid(strs, "2") > 0) {
				this.setSucflag(true);
				return "json";
			}
			this.setSucflag(false);
