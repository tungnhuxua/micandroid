...
public class TemplatesetTAction extends ActionSupport {
	...
	public void findDefaultAllTemplateset() {
		...
		int lineSize = rp;
		total = this.getTemplatesetTServiceImpl().countfindAllTemplatesetT(BaseTools.adminCreateId());
		List<TemplatesetT> list = this.getTemplatesetTServiceImpl().findAllTemplatesetT(currentPage, lineSize, BaseTools.adminCreateId());
		if (list != null) {
	...
	public String findSystemcontent() {
		...
		List<GoodsCategoryT> gclist = this.getGoodsCategoryTServiceImpl().findAllGoodsCategoryBycreatorid(BaseTools.adminCreateId());
		List<ArticleCategoryT> aclist = this.getArticleCategoryTServiceImpl().findAllArticleCategoryBycreatorid(BaseTools.adminCreateId());
		//组织商品分类的所有信息
	...
	public String getAllTemplate() {
		List<TemplateT> list = this.getTemplateTServiceImpl().findAllTemplateWithNoParam(BaseTools.adminCreateId(), "1");
		if (list != null) {
	...
	public String getTemplateOutHtmlPath() {
		...
		List<TemplatesetT> list = this.getTemplatesetTServiceImpl().findAllTemplatesetWithNoParam(BaseTools.adminCreateId());
		if (list != null) {
	...
	public String addTemplatesetT() {
		...
		tt = this.getTemplateTServiceImpl().findTemplateBysign(this.getSign(), "1");
		if (tt != null) {
			tst.setThemeid(tt.getThemeid());
			tst.setThemename(tt.getThemename());
			tst.setStatus(tt.getStatus());
		}
		if (this.getTemplatesetTServiceImpl().addTemplatesetT(tst) > 0) {
			this.setSucflag(true);
			return "json";
		} else {
			this.setSucflag(false);
	...
	public String findTemplatesetTBytsid() {
		...
		if (Validate.StrNotNull(this.getTsid())) {
			bean = this.getTemplatesetTServiceImpl().findTemplatesetTBytsid(this.getTsid());
			if (bean != null) {
				this.setSucflag(true);
				return "json";
			} else {
	...
	public String updateTemplatesetT() {
		...
		if (this.getTemplatesetTServiceImpl().updateTemplatesetT(tst) > 0) {
			this.setSucflag(true);
			return "json";
		}
		this.setSucflag(false);
	...
	public String delTemplatesetT() {
		...
		if (Validate.StrNotNull(this.getTsid())) {
			...
			this.getTemplatesetTServiceImpl().delTemplatesetT(strs);
			this.setSucflag(true);
			return "json";
		}
		this.setSucflag(false);
