...
public class FrontOrderAction extends ActionSupport {
	...
	@Resource(name="logisticsbusinessareaTServiceImpl")
	private LogisticsbusinessareaTServiceImpl logisticsbusinessareaTServiceImpl;
	@Resource(name="vouchersTServiceImpl")
	private VouchersTServiceImpl vouchersTServiceImpl;
	...
	@JSON(serialize = false)
	public LogisticsbusinessareaTServiceImpl getLogisticsbusinessareaTServiceImpl() {
		return logisticsbusinessareaTServiceImpl;
	}

	public void setLogisticsbusinessareaTServiceImpl(LogisticsbusinessareaTServiceImpl logisticsbusinessareaTServiceImpl) {
		this.logisticsbusinessareaTServiceImpl = logisticsbusinessareaTServiceImpl;
	}
	@JSON(serialize = false)
	public VouchersTServiceImpl getVouchersTServiceImpl() {
		return vouchersTServiceImpl;
	}

	public void setVouchersTServiceImpl(VouchersTServiceImpl vouchersTServiceImpl) {
		this.vouchersTServiceImpl = vouchersTServiceImpl;
	}
	@JSON(serialize = false)
	...
	public void GetMyCart(UserT user) {
		...
		if (list != null) {
			...
			Map<String, Object> map = new HashMap<String, Object>();
			map.put("mycart", list);
			ActionContext.getContext().put("cart", map);
	...
	private void GetLogisticsPrice() {
		...
		List<LogisticsbusinessareaT> list = this.getLogisticsbusinessareaTServiceImpl().findAllLogisticsbusinessareaTBylogisticsid(this.getDefaultlogisticsid());
		if (list != null) {
	...
	@Action(value = "InitOrder", results = { 
			@Result(name = "success",location = "/usercenter/order/order.jsp"),
	...
	public String GetVouchersByname() throws ParseException {
		...
		if (user != null) {
			if (Validate.StrNotNull(this.getVouchername())) {
				VouchersT v = this.getVouchersTServiceImpl().findVouchersForHonor(this.getVouchername());
				if (v != null) {
	...
	public void initOrderInfo(UserT user) {
		...
		order.setAmount(Arith.sub(Arith.add(this.getTotal(), this.getFreight()), this.getVouchercontent()));//�����˷Ѻ͵ֿ���Ϣ
		//		}else{
		...
		if (this.getOrderTServiceImpl().addOrder(order) > 0) {
	...
	public String InitAlipayneedInfo() {
		...
		if (user != null) {
			...
			if (!this.isSaddorder()) {
				...
				if (this.getCartTServiceImpl().updateCartStateandOrderidByGoodsidList(this.getCartid().trim(), this.getSerialidorderid(), user.getUserid(), "3") > 0) {
					this.setSupdatecart(false);
					return "json";
				} else {
					this.setSupdatecart(true);
					return "json";
				}
