...
public class MyVouchersAction extends ActionSupport {
	...
	public String findUserVouchers(){
		...
		if (user != null) {
			...
			List<VouchersT> list = this.getVouchersTServiceImpl().findUserVouchers(user.getUserid(), currentPage, lineSize);
			int allRecorders = this.getVouchersTServiceImpl().countfindUserVouchers(user.getUserid());
			//路径获取
