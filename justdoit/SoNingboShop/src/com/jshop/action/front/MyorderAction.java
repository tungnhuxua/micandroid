...
public class MyorderAction extends ActionSupport {
	...
	@Resource(name="logisticsBusinessTServiceImpl")
	private LogisticsBusinessTServiceImpl logisticsBusinessTServiceImpl;
	...
	@JSON(serialize=false)
	public LogisticsBusinessTServiceImpl getLogisticsBusinessTServiceImpl() {
		return logisticsBusinessTServiceImpl;
	}
	public void setLogisticsBusinessTServiceImpl(LogisticsBusinessTServiceImpl logisticsBusinessTServiceImpl) {
		this.logisticsBusinessTServiceImpl = logisticsBusinessTServiceImpl;
	}
	@JSON(serialize=false)
	 ...
	 */
	@Action(value = "findAllUserOrder", results = { 
			@Result(name = "success",type="freemarker",location = "WEB-INF/theme/default/shop/mygoods.ftl"),
			@Result(name = "input",type="redirect",location = "/html/default/shop/login.html?redirecturl=${hidurl}")
	})
	public String findAllUserOrder(){
		...
		if(user!=null){			
			...
			int allRecorders=this.getOrderTServiceImpl().countfindAllOrderByorderstate(user.getUserid(), orderstate, paystate, shippingstate);
			List<OrderT>list=this.getOrderTServiceImpl().findAllOrderByorderstate(currentPage, lineSize, user.getUserid(), orderstate, paystate, shippingstate);
			if(list!=null){
				//路径获取
	 ...
	 * @return
	 */
	@Action(value = "DelOrderByorderid", results = { 
			@Result(name = "success",type="chain",location = "findAllUserOrder"),
			@Result(name = "input",type="redirect",location = "/html/default/shop/login.html?redirecturl=${hidurl}")
	})
	public String DelOrderByorderid(){
		...
		if(user!=null){
			...
			int i=this.getOrderTServiceImpl().delOrderByorderid(user.getUserid(),this.getOrderid().trim() , orderstate);
			return SUCCESS;
		}else{
			return INPUT;
		}
	 ...
	 * @return
	 */
	@Action(value = "ConfirmGoodsReceive", results = { 
			@Result(name = "success",type="chain",location = "findAllUserOrder"),
			@Result(name = "input",type="redirect",location = "/html/default/shop/login.html?redirecturl=${hidurl}")
	})
	 ...
	 * 
	 */
	@Action(value = "findAllUserOrderOn", results = { 
	...
	public String findAllUserOrderOn(){
		...
		if(user!=null){
			...
			allRecorders=this.getOrderTServiceImpl().countfindAllOrderByorderstateForOn(user.getUserid(), orderstate, paystate, shippingstate);
			List<OrderT>list=this.getOrderTServiceImpl().findAllOrderByorderstateForOn(currentPage, lineSize, user.getUserid(), orderstate, paystate, shippingstate);
			if(list!=null){
				//路径获取
				ActionContext.getContext().put("basePath", this.getDataCollectionTAction().getBasePath());
				//获取我的订单
	...
	public void GetDefaultLogistictsBusiness(){
		List<LogisticsBusinessT>list=this.getLogisticsBusinessTServiceImpl().findAllLogisticsBusinessWithoutPage();
		if(list!=null){
			...
			ActionContext.getContext().put("logisticsbusiness", map);
		}
	}
	/**
	 * 获取支付方式
	...
	public void GetDefaultPayment(){
		List<PaymentM>list=this.getPaymentMServiceImpl().findAllPaymentWithoutPage();
		if(list!=null){
			Map<String,Object>map=new HashMap<String,Object>();
			map.put("payment",list);
			ActionContext.getContext().put("payments", map);
		}
	}
	...
	public void GetthisOrderinfo(){
		OrderT o=this.getOrderTServiceImpl().findOrderDetailByorderid(this.getOrderid().trim());
		if(o!=null){
	 ...
	 * @return
	 */
	@Action(value = "InitPayPage", results = { 
			@Result(name = "success",location = "usercenter/order/agorder.jsp"),
			@Result(name = "input",type="redirect",location = "/html/default/shop/login.html?redirecturl=${hidurl}")
	})
	...
	public void AddShippingAddress(String deliveraddressid){
		DeliverAddressT list=this.getDeliverAddressTServiceImpl().findDeliverAddressById(this.getAddressid());
		if(list!=null){
			...
			if(this.getShippingAddressTServiceImpl().addShoppingAddress(s)>0){
	...
	public void CompareShippingAddress(){
		...
		/**获取发货地址表数据，判断传来的收获地址是不是已经在发货地址表中
		 * state="1"表示使用的地址
		 */
		List<ShippingAddressT> list=this.getShippingAddressTServiceImpl().findShippingAddressByDeliveraddressidAndstate(this.getAddressid().trim(), "1",this.getOrderid().trim());
		if(list!=null){
			...
			int i=this.getShippingAddressTServiceImpl().updateShippingAddressByorderandstate(oldorder.getOrderid(), "2");
			//增加地址
	...
	public void UpdateOrderInfo(UserT user){
		...
		try{
			int i=this.getOrderTServiceImpl().updateOrder(order);
	 ...
	 * @return
	 */
	@Action(value = "InitAgAlipayandUpdateOrder", results = { 
	...
	public void GetUserBuyerInfo(String userid){
		UserT user=this.getUsertServiceImpl().findById(userid);
		if(user!=null){
	...
	public void GetOrderGoodsList(String orderid){
		List<CartT>list=this.getCartTServiceImpl().findCartGoodsByOrderid(orderid);
		if(list!=null){
	...
	@Action(value = "InitMyOrdersDetail", results = { 
			@Result(name = "success",location = "/usercenter/businessmag/myordersdetail.jsp"),
