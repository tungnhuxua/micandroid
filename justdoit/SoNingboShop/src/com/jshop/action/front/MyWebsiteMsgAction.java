...
public class MyWebsiteMsgAction extends ActionSupport {
	...
	@Resource(name="websiteMsgTServiceImpl")
	private WebsiteMsgTServiceImpl websiteMsgTServiceImpl;
	@Resource(name="msgtextTServiceImpl")
	private MsgtextTServiceImpl msgtextTServiceImpl;
	...
	@JSON(serialize=false)
	public WebsiteMsgTServiceImpl getWebsiteMsgTServiceImpl() {
		return websiteMsgTServiceImpl;
	}
	public void setWebsiteMsgTServiceImpl(WebsiteMsgTServiceImpl websiteMsgTServiceImpl) {
		this.websiteMsgTServiceImpl = websiteMsgTServiceImpl;
	}
	@JSON(serialize=false)
	public MsgtextTServiceImpl getMsgtextTServiceImpl() {
		return msgtextTServiceImpl;
	}
	public void setMsgtextTServiceImpl(MsgtextTServiceImpl msgtextTServiceImpl) {
		this.msgtextTServiceImpl = msgtextTServiceImpl;
	}
	...
	public String addWebsiteMsgT(){
		...
		if(user!=null){
			...
			if(this.getMsgtextTServiceImpl().addMsgtext(mt)>0){
				...
				if(this.getWebsiteMsgTServiceImpl().addWebsiteMsgT(wm)>0){
					this.setSflag(true);
					return "json";
				}else{
					this.setSflag(false);
	...
	public String findAllWebsiteMsgByToUsername(){
		...
		if(user!=null){
			int currentPage=1;
			int lineSize =15;
			List<WebsiteMsgT> list = this.getWebsiteMsgTServiceImpl().findAllWebsiteMsgByToUsername(currentPage, lineSize, user.getUsername());
			int allRecorders=this.getWebsiteMsgTServiceImpl().countfindAllWebsiteMsgByToUsername(user.getUsername());
			if(list!=null){
				//路径获取
	...
	public String DelWebsiteMsgT(){
		...
		int i=this.getWebsiteMsgTServiceImpl().delWebsiteMsgT(array);
		return SUCCESS;
	}
	...
	public String findMsgtextById(){
		UserT user=(UserT) ActionContext.getContext().getSession().get(BaseTools.USER_SESSION_KEY);
		if(user!=null){
			MsgtextT list=this.getMsgtextTServiceImpl().findMsgtextById(this.getMsgtextid().trim());
			if(list!=null){
				String temp=this.getMsgtextid().trim()+",";
				String []tempid=temp.split(",");
				int i=this.getWebsiteMsgTServiceImpl().updateWebsiteMsgstate(tempid, "1");
				//路径获取
				ActionContext.getContext().put("basePath", this.getDataCollectionTAction().getBasePath());
				//获取收货地址
