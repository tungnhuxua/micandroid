...
public class OrderTAction extends ActionSupport {
	...
	public void finddefaultAllOrder() {
		...
		int lineSize = rp;
		total = this.getOrderTServiceImpl().countfindAllOrderT();
		if (Validate.StrNotNull(sortname) && Validate.StrNotNull(sortorder)) {
			String queryString = "from OrderT order by " + sortname + " " + sortorder + "";
			List<OrderT> order = this.getOrderTServiceImpl().sortAllOrderT(currentPage, lineSize, queryString);
			if (order != null) {
	...
	public void findOrderInfoByorderid(String orderid) {
		int currentPage = page;
		int lineSize = rp;
		total = this.getOrderTServiceImpl().countfindOrderbyOrderid(orderid);
		List<OrderT> order = this.getOrderTServiceImpl().findOrderByOrderid(currentPage, lineSize, orderid);
		if (order != null) {
			this.ProcessOrderList(order);
		}
	}
	...
	public void findOrderInfoByShippingUsername(String shippingusername) {
		int currentPage = page;
		int lineSize = rp;
		total = this.getOrderTServiceImpl().countfindOrderByShippingUsername(shippingusername);
		List<OrderT> order = this.getOrderTServiceImpl().findOrderByOrderid(currentPage, lineSize, orderid);
		if (order != null) {
			this.ProcessOrderList(order);
		}
	...
	public void finddefaultAllTobeShippedOrder() {
		...
		total = this.getOrderTServiceImpl().countfindAllTobeShippedOrders(shippingstate);
		List<OrderT> order = this.getOrderTServiceImpl().findAllTobeShippedOrders(currentPage, lineSize, shippingstate);
		if (order != null) {
			this.ProcessOrderList(order);
		}
	}
	...
	public String UpdateOrderToClose() {
		...
		this.getOrderTServiceImpl().updateOrderPayShippingState(orderid, orderstate, paystate, shippingstate);
		//InitOrdersDetail();
		return "json";
	}
	...
	public String UpdateExpressnumberByOrderId() {
		if (Validate.StrNotNull(this.getExpressnumber())&&Validate.StrNotNull(this.getOrderid())) {
			int i = this.getOrderTServiceImpl().updateExpressnumberByOrderId(this.getOrderid().trim(), this.getExpressnumber().trim());
			return "json";
		}
		return "json";
	...
	public String UpdateInvoicenumberByOrderId() {
		if (Validate.StrNotNull(this.getInvoicenumber())&&Validate.StrNotNull(this.getOrderid())) {
			int i = this.getOrderTServiceImpl().updateInvoicenumberByOrderId(this.getOrderid().trim(), this.getInvoicenumber().trim(), BaseTools.systemtime());
			return "json";
		}
		return "json";
	...
	public String GetAlipayFhNeedParams() {
		...
		LogisticsBusinessT lt = this.getLogisticsBusinessTServiceImpl().findLogisticsBusinessById(o.getLogisticsid());
	...
	public void defaultfindAllhaveshippedOrder(){
		...
		total = this.getOrderTServiceImpl().countAllhaveshippedOrder(shippingstate);
		List<OrderT> order =this.getOrderTServiceImpl().findAllhaveshippedOrder(currentPage, lineSize, shippingstate);
		if(order!=null){
			this.ProcessOrderList(order);
		}
	}
	...
	public void defaultfindAllreturnOrder(){
		...
		int lineSize=rp;
		String orderstate="4";
		total = this.getOrderTServiceImpl().countAllreturnOrder(orderstate);
		List<OrderT> order =this.getOrderTServiceImpl().findAllreturnOrder(currentPage, lineSize, orderstate);
		if(order!=null){
			this.ProcessOrderList(order);
		}
	}
