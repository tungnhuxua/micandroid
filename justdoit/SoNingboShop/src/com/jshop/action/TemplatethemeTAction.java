...
public class TemplatethemeTAction extends ActionSupport {
	...
	public void finddefaultAllTemplatetheme() {
		...
		if (Validate.StrNotNull(sortname) && Validate.StrNotNull(sortorder)) {
			...
			List<TemplatethemeT> list = this.getTemplatethemeTServiceImpl().sortAllTemplatetheme(currentPage, lineSize, BaseTools.adminCreateId(), queryString);
			if (list != null) {
	...
	public void ProcessTemplatethemeList(List<TemplatethemeT> list) {
		total = this.getTemplatethemeTServiceImpl().countfindAllTemplatetheme(BaseTools.adminCreateId());
		rows.clear();
	...
	public String addTemplatetheme() {
		...
		if (this.getTemplatethemeTServiceImpl().checkTemplatethemeBythemenameandsign(this.getThemename(), this.getSign()) > 0) {
		...
		if (this.getTemplatethemeTServiceImpl().addTemplatetheme(tt) > 0) {
	...
	public String delTemplatetheme() {
		if (Validate.StrNotNull(this.getTtid())) {
			...
			for (int i = 0; i < str.length; i++) {
				tt = this.getTemplateTServiceImpl().checkTemplatetheme(str[i]);
				if (tt != null) {
					tip += tt.getThemename() + ",";
					count++;
				} else {
					int j = this.getTemplatethemeTServiceImpl().delTemplatetheme(str[i]);
				}
			}
			if (count == 0) {
				this.setSucflag(true);
	...
	public String findTemplatethemeByttid() {
		...
		if (Validate.StrNotNull(this.getTtid())) {
			bean = this.getTemplatethemeTServiceImpl().findTemplatethemeByttid(this.getTtid());
			if (bean != null) {
				return "json";
			}
		}
	...
	public String updateTemplatetheme() {
		...
		if (this.getTemplatethemeTServiceImpl().checkTemplatethemeBythemenameandsign(this.getThemename(), this.getSign(), this.getTtid()) > 0) {
			this.setSucflag(false);
			return "json";
		}
		TemplatethemeT tt = new TemplatethemeT();
		...
		tt.setCreatorid(BaseTools.adminCreateId());
		tt.setStatus(this.getStatus());
		this.getTemplatethemeTServiceImpl().updateTemplatetheme(tt);
		int k = this.getTemplateTServiceImpl().updateTemplatetBystatus(this.getTtid(), this.getStatus());
		int j = this.getTemplatesetTServiceImpl().updateTemplatesetBystatus(this.getTtid(), this.getStatus());
		this.setSucflag(true);
		return "json";
	...
	public String getAllTemplatetheme() {
		...
		List<TemplatethemeT> list = this.getTemplatethemeTServiceImpl().findAllTemplatethemeWithNoParam(BaseTools.adminCreateId());
		if (list != null) {
	...
	public String updateTemplatethemestatus() {
		...
		if (Validate.StrNotNull(this.getTtid()) && Validate.StrNotNull(this.getStatus())) {
			int i = this.getTemplatethemeTServiceImpl().updateTemplatethemestatus(this.getTtid(), this.getStatus());
			int k = this.getTemplateTServiceImpl().updateTemplatetBystatus(this.getTtid(), this.getStatus());
			int j = this.getTemplatesetTServiceImpl().updateTemplatesetBystatus(this.getTtid(), this.getStatus());
			this.setSucflag(true);
			return "json";
