...
<html xmlns="http://www.w3.org/1999/xhtml">
    <head>
		...
		<title>专题_${showCategory.name}</title>
	...
	<div class="container">
		...
		<div id="content">
			...
			<div id="postion">
				<p>
					<span>您的位置:首页&gt;<a href="${ctx }/special">专题</a>&gt;${showCategory.name}</span>
				</p>
			</div>
			<div id="contentLeft">
				<div id="totop">
				...
				<div id="tabshow">
					...
					<div class="news">
						...
						<div class="newsright">
							<h3><a href="${ctx}/specialcontent.action?article.id=${id}">${title }</a></h3>
					...
					<div style="padding-left:96px">
						...
						<s:if test="page.hasPre">
							<a href="speciallist.action?page.pageNo=${page.prePage}&page.orderBy=${page.orderBy}&page.order=${page.order}&showCategory.id=${showCategory.id}">上一页</a>
						</s:if>
						<ouun:pageNum totalPages="${page.totalPages}" pageNo="${page.pageNo}" url="speciallist.action?page.orderBy=${page.orderBy}&page.order=${page.order}&showCategory.id=${showCategory.id}&page.pageNo=" />
						<s:if test="page.hasNext">
							<a href="speciallist.action?page.pageNo=${page.nextPage}&page.orderBy=${page.orderBy}&page.order=${page.order}&showCategory.id=${showCategory.id}">下一页</a>
						</s:if>
					</div>
					...
					<div id="hotweek" class="votweek">
						<dl>
							...
							<s:if test="#sta.first">
								...
								<dd><a href="${ctx}/specialcontent.action?article.id=${id}"  title="${title }"><ouun:substring length="14" value="${title}"/></a></dd>
							</s:else>
						</s:iterator>
						</dl>
					</div>
