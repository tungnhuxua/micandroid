			...
			<div class="bqmainbh">
				...
				<div class="shoucanweizhi">
					...
					<ul class="scxuangxiang">
						<li>_<a href="${ctx }/blog/blog-show-list.action?tomember.id=${tomember.id}">列表显示</a></li>
						<li>_<a href="${ctx }/blog/blog-show.action?tomember.id=${tomember.id}">图标显示</a></li>
					</ul>
				</div>
				...
				<div class="rizhinr">
					...
					<div class="rizhibiaolujin">
						<ul>
							<li> <s:if test="category!=null">${category.name}</s:if><s:elseif test="logCategory!=null">${logCategory.name}</s:elseif><s:else>${showCategory.name}</s:else></li>
							<li> <s:date name="createDate" format="yyyy-MM-dd hh:mm" />|</li>
							<li> 浏览_${readCount}|</li>
				...
				<div class="qyyear">
					<div class="qyzhongjianyeshu">
						...
						<s:if test="page.hasPre">
							<a href="${ctx }/blog/blog-show-list.action?page.pageNo=${page.prePage}&tomember.id=${tomember.id}">上一页</a>
						</s:if>
						<ouun:pageNum totalPages="${page.totalPages}" pageNo="${page.pageNo}" url="${ctx }/blog/blog-show-list.action?tomember.id=${tomember.id}&page.pageNo=" />
						<s:if test="page.hasNext">
							<a href="${ctx }/blog/blog-show-list.action?page.pageNo=${page.nextPage}&tomember.id=${tomember.id}">下一页</a>
						</s:if>
						</ul>
					</div>
					<div class="qyzhongjianyeshu">__共  ${page.totalCount}  篇秀场</div>
				  </div>
