...
<script type="text/javascript" src="${ctx }/js/blog/circle.js"></script>

<script type="text/javascript">
	var circle_quit = {action:"member-circle!quitCircle.action",msg:"确定要退出该群吗？"};
	function quitCircle(op,id,mcuid) {
		  ...
		  <form  name="circleFrom">
			<div class="mainbh">
				...
				<div class="zhuti">
					...
					<div class="dangannav">
						<ul>
							<li>_<a href="#" style="color: rgb(131, 195, 37);">我加入的圈子</a></li>
							<li>_<a href="member-circle.action" >我管理的圈子</a></li>
						</ul>
					</div>
					...
					<div class="grhaohougl">
						...
						<dl>
							...
							<dd>共发表_99篇<a href="">日志</a>|查看圈子名称的 <a href="">档案</a></dd>
							<dd><a href="#"><%=object[9]%></a>|<a href="javascript:quitCircle(circle_quit,'<%=object[0]%>','<%=object[10]%>');">退出删除</a>|<a  id="upblog_<%=object[10]%>" onclick="a_circleBlogIndex('<%=object[10]%>');" style="cursor: pointer;<%if(object[11].toString().equals("0")){out.print("display:none;");}%> ">取消放置个人首页</a><a  id="unupblog_<%=object[10]%>" onclick="a_uncircleBlogIndex('<%=object[10]%>');" style="cursor: pointer;<%if(object[11].toString().equals("1")){out.print("display:none;");}%>">放置个人首页</a></dd>
						</dl>
						<%} %>
					</div>
					<div class="qyyear">
						<div class="qyzhongjianyeshu">
							<ul>
								...
								<s:if test="page.hasPre">
									<a href="${ctx }/blog/member-mycircle.action?page.pageNo=${page.prePage}">上一页</a>
								</s:if>
								<ouun:pageNum pageNo="${page.pageNo}" url="${ctx}/blog/member-mycircle.action?page.pageNo=" totalPages="${page.totalPages}"/>
								<s:if test="page.hasNext">
									<a href="${ctx }/blog/member-mycircle.action?page.pageNo=${page.nextPage}">下一页</a>
								</s:if>
							</ul>
						</div>
