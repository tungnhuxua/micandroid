			...
			<div class="bqmainbh">
				...
				<div class="shoucanweizhi">
					...
					<ul class="scxuangxiang">
						<li>_<a href="blog-collection-list.action?tomember.id=${tomember.id}">列表显示</a></li>
					</ul>
				</div>
				...
				<div class="shoucanglb">
					...
					<s:iterator value="list">
						<li>
							<a href="${ctx}/blog/blog-content.action?id=${article.id}&&tomember.id=${article.member.id}" target="_blank" title="${article.title}">
										<s:if test="article.imageUri != null && article.imageUri != ''">
											<img src="${ctx}${article.imageUri}" width="100px" height="100px"/>
											</s:if>
										<s:else>
				...
				<div class="qyyear">
					<div class="qyzhongjianyeshu">
						...
						<s:if test="page.hasPre">
							<a href="${ctx }/blog/blog-collection.action?page.pageNo=${page.prePage}&tomember.id=${tomember.id}">上一页</a>
						</s:if>
						<ouun:pageNum totalPages="${page.totalPages}" pageNo="${page.pageNo}" url="${ctx }/blog/blog-collection.action?tomember.id=${tomember.id}&page.pageNo=" />
						<s:if test="page.hasNext">
							<a href="${ctx }/blog/blog-collection.action?page.pageNo=${page.nextPage}&tomember.id=${tomember.id}">下一页</a>
						</s:if>
						</ul>
					</div>
