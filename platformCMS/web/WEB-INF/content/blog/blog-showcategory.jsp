			...
			<div class="bqmainbh">
				...
				<div class="shoucanweizhi">
					...
					<ul class="scxuangxiang">
						<li>_<a href="${ctx }/blog/blog-showcategory-list.action?tomember.id=${tomember.id}&showCategoryT.id=${showCategoryT.id}">列表显示</a></li>
						<li>_<a href="${ctx }/blog/blog-showcategory.action?tomember.id=${tomember.id}&showCategoryT.id=${showCategoryT.id}">图标显示</a></li>
					</ul>
				</div>
				<div class="rizhinr">
					<s:iterator value="page.result">
					<dl>
						<dt><a href="${ctx }/blog/blog-content.action?id=${id}&&tomember.id=${tomember.id}"/><s:if test="imageUri!=null && imageUri != ''"><img src="${ctx}${imageUri}" alt="${title}" width="190" height="190"/></s:if></dt>
						<dd style="font-weight: 700;"><a href="${ctx }/blog/blog-content.action?id=${id}&&tomember.id=${tomember.id}">${title }</a></dd>
						<dd><s:date name="createDate" format="yyyy-MM-dd hh:mm:ss" />_发表于_<s:if test="category!=null">${category.name}</s:if><s:elseif test="logCategory!=null">${logCategory.name}</s:elseif>${showCategory.name}<s:else></s:else></dd>
				...
				<div class="qyyear">
					<div class="qyzhongjianyeshu">
						...
						<s:if test="page.hasPre">
							<a href="${ctx }/blog/blog-showcategory.action?page.pageNo=${page.prePage}&tomember.id=${tomember.id}">上一页</a>
						</s:if>
						<ouun:pageNum totalPages="${page.totalPages}" pageNo="${page.pageNo}" url="${ctx }/blog/blog-showcategory.action?tomember.id=${tomember.id}&page.pageNo=" />
						<s:if test="page.hasNext">
							<a href="${ctx }/blog/blog-showcategory.action?page.pageNo=${page.nextPage}&tomember.id=${tomember.id}">下一页</a>
						</s:if>
						</ul>
					</div>
