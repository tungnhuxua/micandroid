...
<title>资讯-${article.category.name }-${article.title }</title>
...
<div id="postion">
<p><span>您的位置:首页&gt;资讯&gt;${article.category.name }</span></p>
</div>
...
<div id="newsmaincontent">${article.content }</div>
...
<dl>
	...
	<s:if test="preArticle!=null">
		<dd><a href="${ctx}/newscontent?article.id=${preArticle.id}">上一篇_
		${preArticle.title }</a></dd>
	</s:if>
	<s:if test="nextArticle!=null">
		<dd><a href="${ctx}/newscontent?article.id=${nextArticle.id}">下一篇_
...
<dl id="proleft">
	...
	<s:iterator value="TagCorrelationArticle">
		...
		<dd><span><a href="${ctx}/newscontent.action?article.id=${id}">${title}</a></span></dd>
	</s:iterator>
</dl>
...
<div>
<dl>
	<dt>${member.name } , <s:date name="createDate" format="yyyy年MM月dd日" />,<s:date name="createDate" format="HH.mm.ss" /> <security:authorize ifAnyGranted="A_VIEW_ZX"><!--  <span onclick="editComment('${id}');" style="cursor: pointer;">编辑</span>--> <span onclick="delComment('${id }',this);" style="cursor: pointer;">删除</span></security:authorize></dt>
	<dd>${content}</dd>
</dl>
</div>
</div>
...
<!-- 分页验证权限 -->
<security:authorize ifAnyGranted="A_VIEW_ZX"><input type="hidden" id="authority" value="A_VIEW" /></security:authorize>
