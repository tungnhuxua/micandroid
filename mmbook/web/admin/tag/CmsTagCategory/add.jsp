...
<html>
<head>
<title>新增栏目标签页面</title>
<script type="text/javascript">
Ext.namespace("Ext.ux.cmstagcategory");
Ext.ux.cmstagcategory.add<%=u_id%>=Ext.extend(function(){
var panel<%=u_id%> = Ext.getCmp('center-tab-panel');

fun_settest=function(textfield,value){
		frm_add_cmstagcategory<%=u_id%>.form.findField(textfield).setValue(value);
}
 
var getId_ = function(id){
     frm_add_cmstagcategory<%=u_id%>.form.findField("categoryId").setValue(id);
};
/**模板ID*/
...
var comboBoxCheckTree = new Ext.ux.ComboBoxCheckTree({
		...
		tree : {
			...
			rootVisible: true,
			autoScroll:true,  
			loader: new Ext.tree.TreeLoader({dataUrl: 'manage/SitePart/getSynTree.do?v=<%=v_id%>&u=<%=u_id%>',   
...
comboBoxCheckTree.on('change', function(box, newvar, oldvar){
    frm_add_cmstagcategory<%=u_id%>.form.findField('categoryId').setValue(comboBoxCheckTree.getValue()); 
});  
...
var frm_add_cmstagcategory<%=u_id%> = new Ext.FormPanel({
        layout: 'fit',
        labelAlign: 'left',
        frame:true,
        title: '新增',
        ...
        buttons: [{
				...
				if (frm_add_cmstagcategory<%=u_id%>.form.isValid() == false){
	    		    return;
	  		    }else{
	  		       frm_add_cmstagcategory<%=u_id%>.form.findField('cids').setValue(comboBoxCheckTree.getValue());
				}
				Ext.MessageBox.confirm('确认操作','确定保存栏目标签页面?',function(btn){
  	            if (btn == 'yes'){
	  		    frm_add_cmstagcategory<%=u_id%>.form.submit({
	   		    ...
	   		    success:function(form,action){
			          ...
			          if (!(typeof(pag_cmstagcategory<%=index_u_id%>) == "undefined")) {
			            pag_cmstagcategory<%=index_u_id%>.doLoad(0);
			          }
			          //fun_copnetab('<%//=index_u_id%>','管理栏目标签','admin/tag/CmsTagCategory/index.jsp?u=<%=index_u_id%>&v=<%=v_id%>','icon-nav-p1');
	   		          }});
...
var panel_add_cmstagcategory<%=u_id%> = new Ext.Panel({
		renderTo:'add_cmstagcategorydiv<%=u_id%>',
		width:Ext.get("add_cmstagcategorydiv<%=u_id%>").getWidth(),
		height:Ext.get("add_cmstagcategorydiv<%=u_id%>").getHeight(),
		border : false,
		layout : 'border',
        items:[frm_add_cmstagcategory<%=u_id%>]
});
fun_field_fit();
panel<%=u_id%>.on('resize',function(){
    if(Ext.get("add_cmstagcategorydiv<%=u_id%>")){
        var p =panel<%=u_id%>.getActiveTab().getId();
        if(p!='add<%=u_id%>'){
             panel<%=u_id%>.setActiveTab('add<%=u_id%>');
             panel_add_cmstagcategory<%=u_id%>.setWidth(Ext.getCmp('center-tab-panel').getInnerWidth()-2);
             panel_add_cmstagcategory<%=u_id%>.setHeight(Ext.getCmp('center-tab-panel').getInnerHeight()-2);
             fun_field_fit();
...
new Ext.ux.cmstagcategory.add<%=u_id%>();
</script>
</head>
<body>
<div id="add_cmstagcategorydiv<%=u_id%>" style="width:100%;height:100%;"></div>
</body>
</html>
