...
function LightPortal(){this.layout={containerLeft:Light.getContainerLeft(),containerTop:0,containerWidth:Light.getContainerWidth(),bodyLeft:0,bodyTop:80,maxLeft:0,maxTop:40,maxWidth:Light.getContainerWidth(),maxHeight:getWindowHeight(),minHeight:40,scrollbarWidth:Light.getScrollbarWidth(),barWidth:Light.getBarWidth(),footerMarginTop:100}
this.tabs=[];this.allTabs=[];this.allPortlets=[];this.latestAction=Light.getLatestActionObject();this.highLightBar=Light.getHighLightBar();this.progressBar=Light.getProgressBar();this.moveTimer=-1;}
...
LightPortal.prototype.clear=function(){Light.deleteCookie(Light._CURRENT_TAB);}
function joinToGroup2(e,id){if(!Light.isLogin()){Light.portal.latestAction.event=e;Light.portal.latestAction.id=id;Light.portal.latestAction.method="joinToGroup2";Light.showAllLogin(false);return;}
var params="&responseId="+id;Light.ajax.sendRequest(Light._JOIN_TO_GROUP,{parameters:params,onSuccess:responseJoinToGroup2});}
function responseJoinToGroup2(t){var params=t.responseText.split(";");if(params[0]==1){window.location.reload(true);}else{var data={title:L$('CLOSE').title,ok:L$('BUTTON_OK').title,cancel:L$('BUTTON_CANCEL').title,popupName:'joinToGroup'};createPopupDiv('joinToGroup','joinToGroup'+params[0]+'.jst',280,data,null,null);}}
function uploadGroupPictures2(e,groupId,id){var portlet=new PortletPopupWindow(11,300,400,L$('MENU_GROUP_UPLOAD_PICTURE').title,"","","groupPicturePortlet","/groupPicturePortlet.lp",true,false,false,false,false,false,false,0,false,'','','',"groupId="+groupId);portlet.refresh();}
function groupPrivacy(e,groupId,id){var params="&groupId="+groupId;Light.ajax.sendRequest(Light._GET_GROUP_PRIVACY,{parameters:params,onSuccess:responseGroupPrivacy});}
function responseGroupPrivacy(t){var params=t.responseText.split(";");var data={title:L$('CLOSE').title,ok:L$('BUTTON_OK').title,cancel:L$('BUTTON_CANCEL').title,popupName:'groupPrivacy',groupId:params[0],acceptLeaderBulletin:params[1],acceptMembersBulletin:params[2]};createPopupDiv('groupPrivacy','groupPrivacy.jst',280,data,null,null);}
...
params+="&mBulletin="+document.forms['form_'+id]['mBulletin'].value;Light.ajax.sendRequest(Light._SAVE_GROUP_PRIVACY,{parameters:params,onSuccess:responseSaveGroupPrivacy});}
function responseSaveGroupPrivacy(t){var params=t.responseText.split(";");var data={title:L$('CLOSE').title,ok:L$('BUTTON_OK').title,cancel:L$('BUTTON_CANCEL').title,popupName:'savePrivacy'};createPopupDiv('savePrivacy','savePrivacy.jst',280,data,null,null);}
function responseResignGroup(t){Light.reloadPortal();}
...
function viewGroupCategory(e,categoryId,id){if(!Light.isLogin()){Light.portal.latestAction.event=e;Light.portal.latestAction.id=id;Light.portal.latestAction.method="viewGroupCategory";Light.showAllLogin(false);return;}
var portlet=new PortletPopupWindow(11,300,400,L$('MENU_GROUP').title,"","","groupPortlet","/groupPortlet.lp",true,false,false,false,false,false,false,0,false,'','','',"type=joinDetail&parameter="+categoryId);portlet.mode=Light._EDIT_MODE;portlet.refresh();}
...
LightPortalMenuImpl.prototype.renderMenuItemSignIn=function(menuItems,portal){if(Light.hasPermission(Light.permission.PORTAL_SIGN_IN)){if(Light.isLogin()){menuItems.appendChild(Light.newElement({element:'a',onclick:function(){Light.logout();},innerHTML:Light.getMessage('MENU_SIGN_OUT')}));}else{menuItems.appendChild(Light.newElement({element:'a',onclick:function(){Light.showAllLogin(false);},innerHTML:Light.getMessage('MENU_SIGN_IN')}));menuItems.appendChild(Light.newElement({element:'span',innerHTML:Light.getViewTemplate("facebookConnect.view",null)}));menuItems.appendChild(Light.newElement({element:'span',innerHTML:Light.getViewTemplate("twitterLogin.view",null)}));}
return true;}
return false;}
