...
<fmt:message key="portlet.message.delicious.account"/> <a href='javascript:;' onclick="<portlet:renderURL portletMode='EDIT'/>" ><fmt:message key="portlet.label.here"/></a>. 
<br/><br/>
...
<a href='javascript:;' onclick="<portlet:renderURL portletMode='EDIT'/>" ><img src='<%= request.getContextPath() %>/light/images/add.gif' style='border: 0px;' height='16' width='16' align="middle"/><fmt:message key="portlet.button.addBookmark"/></a>
</td>
</tr>
</table>
</light:authenticateOwner>
...
<c:if test='${deliciousTags != null}'>
 <c:forEach var="tag" items="${deliciousTags}" >
  <c:if test='${tag.expanded}'>
    ...
    <span class='portlet-rss' style='margin:20px;'>
	...
	<input type="image" title='<fmt:message key="portlet.label.forwardItem"/>' src="<%= request.getContextPath() %>/light/images/forward.png" style='border: 0px;' height='16' width='16' onClick="javascript:forwardDeliToFriend(event,'<c:out value="${bookmark.id}"/>','<c:out value="${requestScope.responseId}"/>');"/>
	</light:authenticateUser>
    ...
    <a href='<c:out value="${bookmark.url}"/>' 
    	onmouseover="javascript:Light.showSummary(event,'<c:out value="${requestScope.responseId}"/>',<%= org.light.portal.util.Constants._OBJECT_TYPE_DELI_ITEM %>,'<c:out value="${bookmark.id}"/>');"
