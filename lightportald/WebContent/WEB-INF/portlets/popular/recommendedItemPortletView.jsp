...
<form action="<portlet:actionURL />">
	<c:if test='${requestScope.lists != null}'>
	   ...
	   <span class="portlet-rss">
	    ...
	    <a href='javascript:;' onclick="javascript:readRecommendedItem('<c:out value="${item.id}"/>','<c:out value="${requestScope.responseId}"/>');Light.executeRender('<c:out value="${requestScope.responseId}"/>','view','maximized','index=<c:out value="${item.id}"/>');"
